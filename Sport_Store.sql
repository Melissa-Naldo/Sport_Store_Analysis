...
Select round(sum(revenue), 2) as Revenue, round(sum(profit), 2) as Profit, round(sum(revenue)-sum(profit), 2) as Expenses, Count(order_id) as Nº_orders, round(sum(profit)/sum(revenue)*100, 2) as Profit_Margin, sport as Sport
...
Select Month(date) as Month, round(sum(profit), 2) as Profit, round(sum(revenue), 2) as Sales, round(sum(profit)/sum(revenue)*100, 2) as Profit_Margin
From Sport_Store.dbo.orders
...
-- The same query as before but applied to the profit_margin
Select  
row_number() over (order by sum(o.profit)/sum(o.revenue)*100 desc) as revenue_rank,
c.state,
sum(o.revenue) as total_revenue, 
sum(o.profit) as total_profit, 
round(sum(o.profit)/sum(o.revenue)*100, 2) as profit_margin
...
Select rating, sum(revenue) as total_revenue, sum(profit) as total_profit, round(sum(profit)/sum(revenue)*100, 2) as profit_margin
From Sport_Store.dbo.orders
Where rating is not null
Group by rating
order by rating desc
